...
import androidx.compose.foundation.ExperimentalFoundationApi // <-- AÑADE ESTE IMPORT
...
import com.pelipunto.app.ui.home.components.BodyContent
...
// AÑADE ESTA ANOTACIÓN JUSTO AQUÍ
...
fun HomeScreen(
    ...
    LaunchedEffect(key1 = pagerState.currentPage) {
        if (isDragged) {
            ...
            // Asegúrate de que no haya un scroll automático si no hay películas
            if (state.discoverMovies.isNotEmpty()) {
        ...
        AnimatedVisibility(visible = !state.isLoading && state.error == null) {
            BoxWithConstraints(modifier = Modifier.fillMaxSize()) {
                ...
                HorizontalPager(
                    ...
                    // Comprobación para evitar errores si la lista está vacía
                    if (page < state.discoverMovies.size) {
                        if (isAutoScrolling) {
                            AnimatedContent(
                                targetState = page,
                                label = "",
